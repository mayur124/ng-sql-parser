...
var totalBytesConsumed;
(function () {
  var Lexer;

  Lexer = (function () {
    var BOOLEAN, DBLSTRING, LITERAL, COLUMNLITERAL, CUBIXLITERAL, MATH, MATH_MULTI, NUMBER, PARAMETER, SEPARATOR, SQL_BETWEENS, SQL_CONDITIONALS, SQL_FUNCTIONS, SQL_OPERATORS, SQL_SORT_ORDERS, STAR, STRING, SUB_SELECT_OP, SUB_SELECT_UNARY_OP, WHITESPACE;

    function Lexer(sql, opts, componentName) {
      var bytesConsumed, i;
      if (opts == null) {
        opts = {};
      }
      ...
      while (this.chunk = sql.slice(i)) {
        if ([null, undefined].includes(componentName)) {
          bytesConsumed = this.keywordToken() || this.starToken() || this.booleanToken() || this.functionToken() || this.windowExtension() || this.sortOrderToken() || this.seperatorToken() || this.operatorToken() || this.mathToken() || this.dotToken() || this.conditionalToken() || this.betweenToken() || this.subSelectOpToken() || this.subSelectUnaryOpToken() || this.numberToken() || this.stringToken() || this.parameterToken() || this.parensToken() || this.whitespaceToken() || this.literalToken() || this.cubixToken() || this.dataTypeToken() || this.columnToken();
        }
        else {
          bytesConsumed = this.keywordToken() || this.starToken() || this.booleanToken() || this.functionToken() || this.windowExtension() || this.sortOrderToken() || this.seperatorToken() || this.operatorToken() || this.mathToken() || this.dotToken() || this.conditionalToken() || this.subSelectOpToken() || this.subSelectUnaryOpToken() || this.numberToken() || this.stringToken() || this.parameterToken() || this.parensToken() || this.whitespaceToken() || this.dataTypeToken() || this.columnToken() || this.literalToken();
        }
        if (bytesConsumed < 1) {
          throw {
            message: "Unexpected 'End of expression'",
            totalBytesConsumed: totalBytesConsumed + this.chunk.length
          }
        }
        i += bytesConsumed;
        totalBytesConsumed = i;
      }
      this.token('EOF', '');
      this.postProcess();
    }
    ...
    Lexer.prototype.token = function (name, value) {
      return this.tokens.push([name, value, totalBytesConsumed]);
    };
    ...
    Lexer.prototype.columnToken = function () {
      return this.tokenizeFromRegex('COLUMNLITERAL', COLUMNLITERAL);
    };

    Lexer.prototype.dataTypeToken = function () {
      return this.tokenizeFromList('DATATYPE', DATATYPE);
    };
    ...
    SQL_FUNCTIONS = ['AVG', 'COUNT', 'MIN', 'MAX', 'SUM', 'ABS', 'NVL', 'IFNULL', 'CEIL', 'ROUND', 'FLOOR', 'COALESCE', 'LOWER', 'UPPER', 'CONCAT', 'LTRIM', 'RTRIM', 'LPAD', 'RPAD', 'REGEXP_REPLACE', 'DATE_SUB', 'DATE_ADD', 'DATEDIFF', 'TO_DATE', 'CURRENT_DATE', 'TRUNC', 'SUBSTR', 'MONTHS_BETWEEN', 'CAST', 'TRENDNUMBER', 'ISPRESENT', 'EOP', 'STDDEV', 'IF', 'PERIODSTODATE', 'FILTERKPI', 'FILTERAGG', 'ISNULL', 'ISNOTNULL', 'KPI'];

    SQL_SORT_ORDERS = ['ASC', 'DESC'];

    SQL_OPERATORS = ['=', '!=', '>=', '>', '<=', '<>', '<', 'LIKE', 'IS NOT', 'IS', 'BETWEEN', 'NOT LIKE', 'RLIKE', 'NOT RLIKE'];
    ...
    COLUMNLITERAL = /[a-zA-Z]{1}[a-zA-Z0-9_]+[.][a-zA-Z]{1}[a-zA-Z0-9_]+/;
    // COLUMNLITERAL = /[a-zA-Z]+\_?\w+\.\w+\_?\w+/;
    // COLUMNLITERAL = /[a-zA-Z]+\_?\w+\.[\w]*?\_?\w+/;

    CASE_EXPR = ['CASE', 'WHEN', 'THEN', 'ELSE', 'END'];

    AS = /^(AS)(\s`?([a-z_][a-z0-9_]{0,})`?)/i;

    DATATYPE = ['INT', 'LONG', 'DECIMAL', 'DOUBLE', 'DATE', 'TIMESTAMP', 'STRING'];

    return Lexer;

  })();

  exports.tokenize = function (sql, opts, componentName) {
    return (new Lexer(sql, opts, componentName)).tokens;
  };
